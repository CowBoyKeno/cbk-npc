...
- The in-game panel is opened by the configured key in `Config.Commands.panelKey`.
- `Config.Commands.panelCommand` is only the internal id used for the key mapping; it is not the user-facing open command.
