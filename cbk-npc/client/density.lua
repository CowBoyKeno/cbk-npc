...
local function applyDensity(config)
    ...
    local population = config.PopulationDensity or {}
    if population.enabled then
        if isExplicitDensityFactor(population.pedDensity) then
            SetPedDensityMultiplierThisFrame(population.pedDensity)
        end
        if isExplicitDensityFactor(population.scenarioPedDensity) then
            SetScenarioPedDensityMultiplierThisFrame(population.scenarioPedDensity, population.scenarioPedDensity)
        end
        if isExplicitDensityFactor(population.vehicleDensity) then
            SetVehicleDensityMultiplierThisFrame(population.vehicleDensity)
            SetRandomVehicleDensityMultiplierThisFrame(population.vehicleDensity)
        end
        if isExplicitDensityFactor(population.parkedVehicleDensity) then
            SetParkedVehicleDensityMultiplierThisFrame(population.parkedVehicleDensity)
        end
    end
    ...
    if timeBased.enabled then
        ...
        if isExplicitDensityFactor(selected.pedDensity) then
            SetPedDensityMultiplierThisFrame(selected.pedDensity)
        end
        if isExplicitDensityFactor(selected.vehicleDensity) then
            SetVehicleDensityMultiplierThisFrame(selected.vehicleDensity)
            SetRandomVehicleDensityMultiplierThisFrame(selected.vehicleDensity)
        end
