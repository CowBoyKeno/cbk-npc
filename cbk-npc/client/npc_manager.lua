...
local function applyPedBehaviorToEntity(ped, playerPed, settings, accuracy, shootRate)
    ...
    local reactionsEnabled = settings.ignorePlayer ~= true and (
    ...
    if settings.ignorePlayer == true or settings.panicFromGunfire == false then
        hearingRange = 0.0
    end
    ...
    SetPedCombatAttributes(ped, 14, reactionsEnabled)
...
local function shouldRefreshPedAiRuntime(payload)
    ...
    for i = 1, #payload.ops do
        ...
        if type(op) == 'table' and type(op.path) == 'table' then
            ...
            if rootKey == 'NPCBehavior' or rootKey == 'Relationships' then
                return true
            end
        end
    end
